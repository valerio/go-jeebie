...
// CombineBytes combines two 8 util values into a single 16 util value.
// The high byte will be the most significant one.
func CombineBytes(low, high uint8) uint16 {
	return (uint16(high) << 8) | uint16(low)
}

// CheckedAdd adds two 8 util unsigned values and detects if an overflow happened.
...
// CheckedSub subtracts two 8 util unsigned values and detects if a borrow happened.
...
// IsBitSet will check if the util at the specified index is Set to 1 or not.
