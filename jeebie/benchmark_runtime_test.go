...
func BenchmarkEmulatorRuntime(b *testing.B) {
	testCases := []struct {
		name   string
		path   string
		frames int
	...
	for _, tc := range testCases {
		b.Run(tc.name, func(b *testing.B) {
			...
			hBackend := headless.New(tc.frames, headless.SnapshotConfig{})
			...
			// Reset timer to exclude initialization from measurements
			b.ResetTimer()
			b.ReportAllocs()

			// Benchmark loop: measure only the runtime
			for i := 0; i < b.N; i++ {
